...
This begs a question - are darker chocolate bars simply better? For instance, in the eyes of the experts, are chocolate bars with 99% cocoa concentration better than chocolate bars with only 79% cocoa concentration?  
...
include_graphics(here("results/darkness_dist.jpg"))
```
...
darkness_stats <- read_csv(here("results/darkness_stats.csv"))
...
include_graphics(here("results/rating_dist.jpg"))
```
...
Let's explore the relationship between chocolate darkness and expert ratings.

First, we explored the two variables assuming that their relationship is linear.

```{r echo = FALSE, message = FALSE}
# visualize darkness and rating
include_graphics(here("results/scatter_darkness_rating.jpg"))
```

*Figure 3.* Chocolate darkness and expert ratings. Grey section represents 95% confidence intervals for the line of best fit (blue line).
